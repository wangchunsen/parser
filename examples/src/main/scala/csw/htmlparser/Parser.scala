...
object P {
  ...
  val voidElements = Set("area", "base", "br", "col", "embed",
  ...
  def tagName: Parser[String] =
    p(charsWhileIn("a-z0-9A-Z_").min(1).cap)
  ...
  def text: Parser[Text] = p(charsWhile(c => c != '<').min(1).cap map Text)
  ...
  def element: Parser[Element] =
    p("<" ~> tagName ~ attributes ~ closeType flatMap (t => {
      val (tagName, attrs, closed) = t

      def element(children: Seq[Node] = Seq.empty): Element = Element(tagName = tagName, attributes = ListMap(attrs: _*), children = children)

      if (closed || voidElements(tagName)) pass(element())
      else {
        val childrenNodes = node.rep
        p(childrenNodes <~ s"</$tagName>") map { nodes =>
...
object Parser extends App {
  print(parse("<h1>hahahah<a></h11>", P.element ~ end))
  println("Done")
}